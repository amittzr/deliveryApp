...
exports.geocodeAndCreatePackage = async function (req, res) {
    try {
        ...
        let customer = await Customer.findById(packageData.customer_id);

        if (!customer) {
            // // יצירת לקוח חדש אם לא קיים
            // customer = new Customer({
            //     _id: packageData.customerId,
            ...
            return res.status(400).json({ error: 'Customer not found in database' });
        }
        ...
        const newPackage = new Package({
            ...
            eta: new Date(packageData.eta * 1000),
            start_date: new Date(packageData.start_date * 1000),
            status: packageData.status || 'packed',
        ...
        res.status(201).json({ message: 'Package created successfully', id: newPackage._id });
